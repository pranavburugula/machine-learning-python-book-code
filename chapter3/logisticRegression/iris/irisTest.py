...
iris_y = iris.target
iris_lr = LogisticRegression(C=1000.0, random_state=0)
...
print("Training...")
iris_lr.fit(X_train_std, y_train)
print("Done!")
...
def classify_iris():
    iris_predict = iris_lr.predict(X_test_std)
    print(y_test)
    print(iris_predict)
    plot_decision_regions(X=X_combined_std, y=y_combined, classifier=iris_lr, test_idx=range(105,150))
