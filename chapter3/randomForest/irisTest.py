...
iris_y = iris.target
iris_forest = RandomForestClassifier(criterion='entropy', n_estimators=10, random_state=1, n_jobs=2)
...
def classify_iris():
    iris_predict = iris_forest.predict(X_test)
    print(y_test)
    print(iris_predict)
    plot_decision_regions(X=X_combined, y=y_combined, classifier=iris_forest, test_idx=range(105,150))
